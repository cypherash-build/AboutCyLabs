...
const Hyperspeed = ({ effectOptions = {
  ...
  useEffect(() => {
    ...
    const roadMarkings_fragment = `
      ...
      float shoulderLines = step(1. - shoulderLinesWidth, uv.x) + step(1. - shoulderLinesWidth, 1. - uv.x);
      
      float brokenLines = 0.;
      for(float i=1.; i < uLanes; i++){
          float laneX = i * laneWidth;
          brokenLines += step(laneX - brokenLineWidth/2., uv.x) * step(uv.x, laneX + brokenLineWidth/2.)* step(laneEmptySpace, fract(uv.y * 10.0));
      }
